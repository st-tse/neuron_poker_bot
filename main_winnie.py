...
Usage:
  ...
  main.py selfplay dpg_train [options]
...
def command_line_parser():
    ...
    if args['selfplay']:
        ...
        if args['random']:
            ...
            runner.dpg_train(num_par_agents, model_name)
...
class SelfPlay:
    ...
    def dpg_train(self, num_par_agents, model_name):
        """Implementation of deterministic policy gradient."""
        from agents.agent_consider_equity import Player as EquityPlayer
        from dpg_agent import Player as DPGPlayer
        from agents.agent_random import Player as RandomPlayer
        ...
        # env.add_player(RandomPlayer())
        # # shell is used for callback to keras rl
        # env.add_player(PlayerShell(name='dpg', stack_size=self.stack))
